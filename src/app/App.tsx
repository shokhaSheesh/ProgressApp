              ...
              <div key={ri} className="flex gap-2.5">
                {row.map((cat) => (
                  <button key={cat.label} onClick={() => { setActiveCatPage({ label: cat.label, icon: cat.icon }); onSubPageChange(true); }}
                    ...
                    <div className={`w-16 h-16 rounded-full overflow-hidden flex items-center justify-center transition-all border-2 border-transparent hover:border-primary text-primary ${cat.img ? "bg-white hover:bg-white" : "bg-[#F4F5F7] hover:bg-primary/10"}`}>
                      {cat.img
                        ? <img src={cat.img} alt={cat.label} className="w-15 h-15 object-contain" />
